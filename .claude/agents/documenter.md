...
`.claude/contexts/` is created by `/bootstrap` per project. Where it does not exist — this
repository has none — there is nothing to update and nothing to create.
...
This table used to name `seller`, `buyer`, `campaigns` and `billing` with their internal
layout (`buyer/keyboards`, `buyer/states.py`) — the domains of one specific product. Read
literally in any other project, it sends you looking for directories that do not exist and
teaches you that product's vocabulary. Read the repository you are in.
...
where it is absent — this repository included — there is nothing to check and nothing to
create.
...
This table used to enumerate `billing`, `campaigns`, `seller`, `buyer` and `outreach` —
the domains of a different project. A documenter reading it looked for glossaries that
were never going to exist here, and imported another product's domain names while doing it.
