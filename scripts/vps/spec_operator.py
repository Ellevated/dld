...
def _set_status(project: Path, spec_id: str, target: str, reason: str | None) -> int:
    """Apply target status to spec + backlog via callback._git_commit_push."""
    spec_path = _find_spec(project, spec_id)
    if spec_path is None:
        print(f"operator: spec not found: {project}/ai/features/{spec_id}*.md", file=sys.stderr)
        return 3
    ...
    if not ok:
        print(f"operator: failed to apply spec status (target={target})", file=sys.stderr)
        return 3
    ...
    if backlog_head is not None:
        ok_b, new_backlog = callback._apply_backlog_status(backlog_head, spec_id, target)
