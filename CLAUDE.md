...
| Работа | Стандарт |
| ------ | -------- |
| постановка задачи: спека, карточка, ТЗ, критерий готовности; приёмка: закрыть спеку или PR, критик, ревью результата; что нести Олегу, эскалация; документ наружу | `rezultat.md` |
| замер, eval, A/B, эксперимент, «стало ли лучше», сравнить, выбрать или сменить модель, базовая линия, журнал конвейера, проход «что снять», повысить уровень доверия, улучшение по данным, `/reflect` | `zamer.md` |
| конвейер, пайплайн, автоматизация, оркестратор, диспетчер, крон, хук, гейт, статусы, переходы, ретраи, дубли прогонов, дожим, очередь, «почини пайплайн» | `agent-vmesto-relsov.md` |
| подключить сервис, MCP, API, CLI, плагин; MCP не подключился; зависимость и её версия (`npx`, `uvx`); браузерная автоматизация; выбрать модель или провайдера; ключ, токен, секрет; права агента; данные наружу; свой инструмент | `instrumenty.md` |
| создать или править скилл, промпт агента, CLAUDE.md, AGENTS.md, `.claude/rules/` | `skilly.md` |
| завести стандарт, методичку, «как у нас принято делать X» | `o-standartah.md` |

Канон таблицы и карта всех её копий — `standards/README.md` в MeMyselfAndI.
