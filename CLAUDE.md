...
| Работа | Стандарт |
| ------ | -------- |
| поставить задачу: спека, карточка, ТЗ, критерий готовности; закрыть задачу, спеку или PR; что нести Олегу; документ наружу | `rezultat.md` |
| замер, eval, сравнить или выбрать модель, A/B, эксперимент, «стало ли лучше», базовая линия, журнал конвейера, улучшение по данным, `/reflect` | `zamer.md` |
...
| подключить сервис, MCP, API, CLI, плагин, браузерная автоматизация, выбрать модель или провайдера, ключ, токен, секрет, права агента, свой инструмент | `instrumenty.md` |
