...
| BUG-974 | Autostash silently overwrites fresh callback Status commits — pop без конфликта возвращает stale DLD-CALLBACK-MARKER блоки поверх HEAD. Свидетельства 2026-05-15: dowry:FTR-429 ×6 (~$4 burned), awardybot:FTR-1012/1018. Fix: post-pop восстанавливать callback-marker блоки из HEAD (ADR-018 — callback = sole writer). | done | P1 | [spec](features/BUG-974-2026-05-15-autostash-callback-marker-overwrite.md) |
| ARCH-975 | **Orchestrator Lifecycle State SoT migration** — Council #2 решение (3 reject Issues / 1 approve; convergence Pragmatist+Security+Product на Option D). Перенести Status из markdown (backlog.md + spec.md DLD-CALLBACK-MARKER) в per-spec `ai/lifecycle/{spec_id}.yaml`. Callback = единственный writer через atomic git plumbing (private GIT_INDEX_FILE + CAS update-ref + commit-tree). No-dirty-WT invariant убирает корень BUG-974. Markdown → auto-rendered read-only view. Удаляет ~700 LOC (marker_utils, verify_status_sync 284 LOC, autostash dance 81 LOC, _restore_callback_markers_from_head 54 LOC, 714 LOC тестов). **ACTION REQUIRED:** human review перед autopilot (R1 P0). | blocked | P0 | [spec](features/ARCH-975-2026-05-16-orchestrator-lifecycle-file-sot.md) |
