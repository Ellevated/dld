...
| ARCH-186 | **Orchestrator Lifecycle State SoT migration** — Council #2 решение (3 reject Issues / 1 approve; convergence Pragmatist+Security+Product на Option D). Перенести Status из markdown (backlog.md + spec.md DLD-CALLBACK-MARKER) в per-spec `ai/lifecycle/{spec_id}.yaml`. Callback = единственный writer через atomic git plumbing (private GIT_INDEX_FILE + CAS update-ref + commit-tree). No-dirty-WT invariant убирает корень BUG-185. Markdown → auto-rendered read-only view. Удаляет ~700 LOC (marker_utils, verify_status_sync 284 LOC, autostash dance 81 LOC, _restore_callback_markers_from_head 54 LOC, 714 LOC тестов). **ACTION REQUIRED:** human review перед autopilot (R1 P0). | queued | P0 | [spec](features/ARCH-186-2026-05-16-orchestrator-lifecycle-file-sot.md) |
