...
| TECH-169 | Orchestrator circuit-breaker on mass-demote (>3 demotes/10min → pause + alert) | done | P0 | [spec](features/TECH-169-2026-05-02-orchestrator-circuit-breaker.md) |
...
| TECH-175 | Spark spec template hardening — DO-NOT-REMOVE markers + schema versioning | done | P2 | [spec](features/TECH-175-2026-05-02-spark-spec-template-hardening.md) |
| TECH-176 | IMPL_GUARD: detect "already merged before started_at" → auto-close instead of demote | done | P1 | [spec](features/TECH-176-2026-05-04-guard-already-merged-detection.md) |
...
| TECH-184 | Hermes intake contract — full status lifecycle SSOT (ai/inbox/README.md) + ADR-022 + OpenClaw→Hermes rename + scan_inbox regression tests | done | P1 | [spec](features/TECH-184-2026-05-14-hermes-intake-contract.md) |
| BUG-185 | Autostash silently overwrites fresh callback Status commits — pop без конфликта возвращает stale DLD-CALLBACK-MARKER блоки поверх HEAD. Свидетельства 2026-05-15: dowry:FTR-429 ×6 (~$4 burned), awardybot:FTR-1012/1018. Fix: post-pop восстанавливать callback-marker блоки из HEAD (ADR-018 — callback = sole writer). | done | P1 | [spec](features/BUG-185-2026-05-15-autostash-callback-marker-overwrite.md) |
| ARCH-186 | **Orchestrator Lifecycle State SoT migration** — Council #2 решение (3 reject Issues / 1 approve; convergence Pragmatist+Security+Product на Option D). Перенести Status из markdown (backlog.md + spec.md DLD-CALLBACK-MARKER) в per-spec `ai/lifecycle/{spec_id}.yaml`. Callback = единственный writer через atomic git plumbing (private GIT_INDEX_FILE + CAS update-ref + commit-tree). No-dirty-WT invariant убирает корень BUG-185. Markdown → auto-rendered read-only view. Удаляет ~700 LOC (marker_utils, verify_status_sync 284 LOC, autostash dance 81 LOC, _restore_callback_markers_from_head 54 LOC, 714 LOC тестов). **ACTION REQUIRED:** human review перед autopilot (R1 P0). | done | P0 | [spec](features/ARCH-186-2026-05-16-orchestrator-lifecycle-file-sot.md) |
| ARCH-187 | **Lifecycle Write Identity Enforcement (ADR-023 gap)** — closure для retry-loop bookkeeping. `spec_operator.py` — легитимный side door с hardcoded `by: operator` (доступен агентам через bash). reflect/autopilot пишут lifecycle.yaml через прямой `git commit` поверх stale WT, откатывая callback'овый blocked → queued. 4 слоя fix: identity validation в `write_lifecycle(by=...)`, pre-commit hook на ai/lifecycle/, WT sync через `git checkout-index` после write, skill audit (autopilot-git.md 5.2 / qa --by flag / template sync). | done | P0 | [spec](features/ARCH-187-2026-05-20-lifecycle-write-identity-enforcement.md) |
| BUG-188 | **claude-runner false-fail on post-result SDK exception** — pueue #265/269 (turns=43, cost=$3-6, cache_hit=0.95) завершались exit 1 потому что SDK бросал Exception ПОСЛЕ ResultMessage(is_error=False), а `claude-runner.py:234` затирал успешный exit_code=0 на 1. Callback ставил blocked, ARCH-187 drift вытаскивал в queued, orchestrator ре-диспатчил уже-сделанную работу ещё на 43 turns. 4 слоя fix: track result_received/result_is_error, capture stderr субпроцесса, autopilot early-exit при merged commits, telemetry table `sdk_post_result_errors`. | done | P0 | [spec](features/BUG-188-2026-05-20-claude-runner-post-result-exception-false-fail.md) |
| TECH-189 | **P0 hardening cluster — callback/lifecycle/orchestrator contour (post-/architect)** — 9 независимых P0 фиксов после /architect retrofit сессии: testpaths CI fix, DB-isolation autouse fixture, zombie cleanup (spec_lint.py + DLD-CALLBACK-MARKER refs), BOOTSTRAP_ANOMALY threshold log, _push_best_effort DEBUG→WARNING, GROWTH в _SPEC_ID_RE, lifecycle._run() timeout=30, heartbeat file + monitor, reconcile_orphans by="orchestrator". Prerequisite gate перед Alt C Wave 1. | queued | P0 | [spec](features/TECH-189-2026-05-23-p0-hardening-cluster.md) |
| ARCH-190 | **gate-daemon shadow implementation (Alt C Wave 1 — MP-001)** — извлекает gate-логику callback в отдельный systemd --user demon (60s polling, multi-project, JSONL shadow + SQLite health). Pure-function `gate_logic.py` (parse_allowed_files, match_subject, find_implementation_commit) с subject-only matching (НЕ bare `--grep`, Devil/TECH-177 mitigation). Wave 1 invariant: SHADOW_ONLY_MODE=True, `_ALLOWED_WRITERS` не содержит "gate-daemon" — zero writes to lifecycle. 7 задач, 8 файлов. ~380 LOC новых + 2 модификации (schema + db helpers + systemd unit). Cutover в Wave 3. | queued | P0 | [spec](features/ARCH-190-2026-05-23-gate-daemon-shadow.md) |
| TECH-191 | **Autopilot worktree `git worktree add -b` без явного base ref — silent main-contamination** — `git worktree add ".worktrees/{ID}" -b "{type}/{ID}"` без последнего аргумента → база = current cwd HEAD. Если что-то оставило cwd на main (broken state / параллельная сессия / orchestrator импровизация) — ветка от main, PHASE 3 merge тащит main-only коммиты (dependabot bumps, release merge-backs) в develop. Real incident: awardybot TECH-1063 (2026-05-24) — потащило setup-node v4→v6 + scp-action 0.1.7→1.0.0, recovered через reset+cherry-pick. Fix: `git fetch origin develop` + явный `origin/develop` base ref в 2 файлах × 2 копии (template/ + .claude/). Anti-pattern класса ADR-021/TECH-182 «implicit assumption baked into tooling». | draft | P1 | [spec](features/TECH-191-2026-05-24-autopilot-worktree-implicit-base-ref.md) |
| BUG-192 | **Callback gate `_subject_implements` слепа к реальным форматам autopilot-коммитов** — 5 ложных demote за ночь 24-05/25 (FTR-1076, FTR-1077, FTR-1078, FTR-441, FTR-182), при том что код реально написан и merge'нут. Симуляция на коммитах FTR-1076: 0/6 субъектов матчатся. 3 дефекта: (1) `_subject_implements`/`match_subject` принимают только UPPERCASE spec_id в scope + жёсткий `^merge\s+SPEC\b` — пропускает `feat(ftr-1076):` и `Merge feature/FTR-1076:`; (2) autopilot не имеет SSOT для commit format, модель выбирает scope сама (lowercase / component / trailing); (3) [→ ARCH-193 deferred] self-promote race через lifecycle.write_lifecycle. Fix: 2 regex патча в callback.py + gate_logic.py (case-insensitive scope, merge branch-prefix) + новый раздел `## Commit Format` в .claude/agents/coder.md (sync template). TECH-177 anti-body invariant сохранён (no trailing fallback). | queued | P0 | [spec](features/BUG-192-2026-05-25-callback-subject-implements-blind-to-real-formats.md) |
| TECH-194 | **ARCH-193 follow-up — worktree hook gap + WT sync env loss + callback exit-code parser** — три незакрытых слоя после ARCH-193: (C1) `core.hooksPath` relative → hook не срабатывает в worktrees; (C2) pre-commit wrapper fail-open при отсутствии guard.mjs в ветке worktree; (C3) audit bypass gap — event_writer.py вызывается через CWD-relative path, не найден в awardybot/dowry; (D) `checkout-index` без `env=env` → WT не синхронизируется после write_lifecycle + create_initial (файлы в HEAD, ` D` в WT); (E) callback диспатчит qa+reflect в Step 6 до проверки task_status в Step 7 — $2.61/blocked задачу. Подтверждено code inspection + прямым тестом bypass. | queued | P0 | [spec](features/TECH-194-2026-05-25-arch193-followup-worktree-wtsync-callback-parser.md) |
