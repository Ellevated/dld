...
**Граница среза — поле в run-логе, а не дата.** 23.09 в момент пуша на VPS уже шли прогоны
plpilot и dowry-mc на старом раннере, а оркестратор подтягивает репо dld только тогда, когда
в нём самом не идёт агент.
`--split 2026-09-23` засчитает их в «после». В срез «после» брать только логи, где есть
поле `system_prompt`: у старого раннера его нет вовсе.
